...
The first step is to select values of M that are predicted by E

```{r}
COLS <- alpha(colorRampPalette(c("seashell","tomato3"))(30),0.8)
pM_E + geom_point(data = pred_M_E, aes(y = estimate, x = E, color = O), size = 4) +scale_color_gradientn(colours = COLS)
```
